...
Insgesamt gab es je Implementierungsvariante 3 Durchläufe bei denen jeweils 10000 `GET` Requests auf den `GetAllUsers` Endpunkt durchgeführt wurden. In der Datenbank sind dafür 500 Datensätze angelegt.
...
----
=== Classic (imperativ) ===
cmd: ./run.sh "http://<ip>:8080/user" 10000
...
=== Mutiny (reaktiv) ===
cmd: ./run.sh "http://<ip>:8081/user" 10000
